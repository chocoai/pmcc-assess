...
import com.copower.pmcc.assess.dal.basis.custom.mapper.CustomReportLandAssociationMapper;
 ...
 */
@Service
public class CustomReportLandAssociationService {
    @Autowired
    ...
    @Autowired
    private CustomReportLandAssociationMapper customReportLandAssociationMapper;
    @Autowired
    ...
    public BootstrapTableVo getCustomReportLandAssociationList(String projectName, String numberValue, String unitName,
        ...
        List<CustomReportLandAssociation> customNumberRecordList = null;
        //土地
        BaseProjectClassify projectCategory = baseProjectClassifyService.getCacheProjectClassifyByFieldName(AssessProjectClassifyConstant.SINGLE_HOUSE_LAND_CERTIFICATE_TYPE_SIMPLE);
        ...
        customNumberRecordList = customReportLandAssociationMapper.getCustomReportLandAssociationList(projectName, projectCategoryId, resultId, numberValue, unitName, startDate, endDate);

        List<CustomReportLandAssociation> vos = LangUtils.transform(customNumberRecordList, o -> getCustomReportLandAssociation(o));
        vo.setTotal(page.getTotal());
        vo.setRows(CollectionUtils.isEmpty(vos) ? new ArrayList<CustomReportLandAssociation>() : vos);
        return vo;
    }

    public CustomReportLandAssociation getCustomReportLandAssociation(CustomReportLandAssociation data) {
        if (data == null) {
            return null;
        }
        CustomReportLandAssociation vo = new CustomReportLandAssociation();
    ...
    public void export(HttpServletResponse response, String projectName, String numberValue, String unitName,
        ...
        String[] title = {"估价报告编号", "项目名称", "业务来源", "估价目的", "估价方法",
                "估价作业开始日", "估价作业结束日", "估价时点", "土地面积（㎡）", "评估总值（万元）", "评估单价（元）", "估价对象位置",
        ...
        for (int i = 0; i < vos.size(); i++) {
            ...
            CustomReportLandAssociation association = vos.get(i);
            ...
            row.createCell(9).setCellValue(String.valueOf(association.getAssessTotal() == null ? "" : association.getAssessTotal())); // 评估总值
            row.createCell(10).setCellValue(String.valueOf(association.getPrice() == null ? "" : association.getPrice())); // 评估单价
            row.createCell(11).setCellValue(association.getSeat()); // 估价对象位置
            row.createCell(12).setCellValue(association.getConsignor()); // 委托人
            row.createCell(13).setCellValue(association.getCsAddress()); // 委托人地址
            row.createCell(14).setCellValue(association.getCsPostcode()); // 委托人邮编
            row.createCell(15).setCellValue(association.getCsPhnoe()); // 委托人电话
            row.createCell(16).setCellValue(association.getFirstAppraiser()); // 第一报告人
            row.createCell(17).setCellValue(association.getFirstRegistrationNumber()); // 第一报告人注册号
            row.createCell(18).setCellValue(association.getParticipationAppraiser()); // 参与报告人
            row.createCell(19).setCellValue(association.getRegistrationNumber()); // 注册号
            row.createCell(20).setCellValue(association.getContractPrice()); // 收费
        }

        OutputStream os = response.getOutputStream();
        try {
            this.setResponseHeader(response, "中国土协报表.XLS");
            wb.write(os);
