...
public class BasicHouseEquipmentController {
    ...
    public HttpResult saveAndUpdateBasicHouseEquipment(BasicHouseEquipment basicHouseEquipment){
        try {
            return HttpResult.newCorrectResult(200,basicHouseEquipmentService.saveAndUpdateBasicHouseEquipment(basicHouseEquipment,true));
        } catch (Exception e) {
            logger.error(String.format("Server-side exception:%s",e.getMessage()),e);
            return HttpResult.newErrorResult(500,e.getMessage());
        }
    ...
    public BootstrapTableVo getBootstrapTableVo(BasicHouseEquipment basicHouseEquipment, @RequestParam(required = true, name = "approval", defaultValue = "false") Boolean approval){
        try {
