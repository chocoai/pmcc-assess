...
public class GenerateBaseDataService {
            ...
            switch (baseReportFieldEnum) {
                ...
                case LandArea: {
                    ...
                    if (declareRealtyLandCert.getUseRightArea() != null) {
                        bigDecimal = declareRealtyLandCert.getUseRightArea();
                    }
                    if (bigDecimal == null) {
                        if (declareRealtyRealEstateCert.getEvidenceArea() != null) {
                            bigDecimal = declareRealtyRealEstateCert.getEvidenceArea();
                        }
                    }
                    if (bigDecimal == null) {
                        if (declareRealtyRealEstateCert.getUseRightArea() != null) {
                            bigDecimal = declareRealtyRealEstateCert.getUseRightArea();
                        }
                    }
                    if (bigDecimal == null) {
                        if (declareRealtyHouseCert.getEvidenceArea() != null) {
                            bigDecimal = declareRealtyRealEstateCert.getEvidenceArea();
                        }
                    }
                    if (bigDecimal != null) {
                        map.put(generateCommonMethod.parseIntJudgeNumber(schemeJudgeObject.getNumber()), generateCommonMethod.getBigDecimalRound(bigDecimal, 2, false));
                    }
                }
                break;
                default:
    ...
    public String getHotTip(List<KeyValueDto> keyValueDtoList) throws Exception {
        ...
        String localPath = getLocalPath();
        DocumentBuilder documentBuilder = getDefaultDocumentBuilderSetting(doc);
        documentBuilder.insertHtml(AsposeUtils.getWarpCssHtml(getHotTip2(true, false), keyValueDtoList), false);
        doc.save(localPath);
        return localPath;
    }

    public String getHotTip2(boolean html, boolean blankSpace) throws Exception {
        ...
        final String repeatString = " ";
        {
            ...
            if (blankSpace) {
                stringBuilder.append(String.format("%d、%s", row + 1, StringUtils.join(stringSet, "，")));
                stringBuilder.append(StringUtils.repeat(repeatString, repeat));
            }
            stringSet.clear();
            row++;
        }
     ...
     * @throws Exception
     */
    public String getLiquidityRisk2(List<KeyValueDto> keyValueDtoList) throws Exception {
        Document document = new Document();
        String localPath = getLocalPath();
        DocumentBuilder builder = getDefaultDocumentBuilderSetting(document);
        String result = "/";
    ...
    public String getSchemeJudgeObjectCertNameList2() throws Exception {
        StringBuilder stringBuilder = new StringBuilder(8);
        List<SchemeJudgeObject> judgeObjectList = getSchemeJudgeObjectList();
        Map<SchemeJudgeObject, String> map = Maps.newHashMap();
        for (SchemeJudgeObject schemeJudgeObject : judgeObjectList) {
        ...
        if (!map.isEmpty()) {
            map.forEach((schemeJudgeObject, s) -> {
                DeclareRecord declareRecord = declareRecordService.getDeclareRecordById(schemeJudgeObject.getDeclareRecordId());
                if (map.size() != 1) {
                    stringBuilder.append(" ").append(generateCommonMethod.getSchemeJudgeObjectShowName(schemeJudgeObject));
                }
                if (Objects.equal(declareRecord.getDataTableName(), FormatUtils.entityNameConvertToTableName(DeclareRealtyRealEstateCert.class))) {
                ...
                if (Objects.equal(declareRecord.getDataTableName(), FormatUtils.entityNameConvertToTableName(DeclareRealtyHouseCert.class))) {
                    ...
                    if (declareRealtyLandCert != null && StringUtils.isNotEmpty(declareRealtyLandCert.getLandCertName())) {
                        stringBuilder.append(String.format("房屋所有权证号:%s", s));
                        stringBuilder.append(";").append(String.format("国有土地使用权证号:%s", declareRealtyLandCert.getLandCertName()));
                    } else {
                        stringBuilder.append(s);
                    }
                }
                stringBuilder.append("\n").append(StringUtils.repeat("\r", 25));
            });
        }
    ...
    public String getAssessPriceClassification() throws Exception {
        ...
        if (!map2.isEmpty()) {
            for (Map.Entry<SchemeJudgeObject, KeyValueDto> entry : map2.entrySet()) {
                BigDecimal one = new BigDecimal(entry.getValue().getKey());
                BigDecimal two = new BigDecimal(entry.getValue().getValue());
                String value = one.multiply(two).divide(new BigDecimal(10000)).setScale(2, BigDecimal.ROUND_DOWN).toString();
        ...
        if (!map.isEmpty()) {
            if (map.size() == 1) {
                ...
                map.entrySet().forEach(entry -> {
                    ...
                    AsposeUtils.insertHtml(documentBuilder, generateCommonMethod.getSongWarpCssHtml(value), false);
                });
            }
        }
        document.save(localPath);
