...
public class GenerateBaseDataService {
    ...
    public String getHotTip() throws Exception {
        ...
        }
        {
            ...
            if (CollectionUtils.isNotEmpty(integerList)) {
                stringSet.add(String.format("%s号有%s", generateCommonMethod.convertNumber(integerList.stream().distinct().collect(Collectors.toList())), projectClassify.getName()));
        ...
        for (int i = 0; i < rowLength; i++) {
            ...
            for (int j = 0; j < cellLength + 1; j++) {
                ...
                if (i == 0) {
                    ...
                    switch (j) {
                        case 0:
                            builder.writeln(generateCommonMethod.getSchemeJudgeObjectShowName(schemeJudgeObject));
                            break;
                        case 1:
    ...
    public Document getUnitPriceAdjustmentDocument(List<SchemeJudgeObjectVo> schemeJudgeObjectList) throws Exception {
        ...
        if (CollectionUtils.isNotEmpty(schemeJudgeObjectList)) {
            generateCommonMethod.writeWordTitle(builder, Lists.newLinkedList(Lists.newArrayList("估价编号", "权证号", "楼层", "房号", "价格", "因素")));
