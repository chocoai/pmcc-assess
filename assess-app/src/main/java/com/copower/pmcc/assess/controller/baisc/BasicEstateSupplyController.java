...
public class BasicEstateSupplyController {
    ...
    public HttpResult saveAndUpdateBasicEstateSupply(BasicEstateSupply basicEstateSupply){
        try {
            return HttpResult.newCorrectResult(200,basicEstateSupplyService.saveAndUpdateBasicEstateSupply(basicEstateSupply,true));
        } catch (Exception e) {
            logger.error(String.format("Server-side exception:%s",e.getMessage()),e);
            return HttpResult.newErrorResult(500,e.getMessage());
        }
    ...
    public BootstrapTableVo getBootstrapTableVo(BasicEstateSupply basicEstateSupply, @RequestParam(required = true, name = "approval", defaultValue = "false") Boolean approval){
        try {
