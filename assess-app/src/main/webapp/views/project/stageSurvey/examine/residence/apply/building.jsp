...
<script type="application/javascript">
    ...
    (function () {
        ...
        var floorPlanAttachmentId = null;//临时存储的附件id
        var figureOutsideAttachmentId = null;//临时存储的附件id
        var floorAppearanceFigureAttachmentId = null;//临时存储的附件id
        ...
        examineBuilding_.prototype = {
            ...
            //获取临时存储的附件id
            getFloorPlanAttachmentId: function () {
                return floorPlanAttachmentId;
            },
            //设置临时存储的附件id
            ...
            navButtonBuild: {
                ...
                copyData: function (target, identifierNumber) {
                    ...
                    if ("buildingNumber" in newObj) {//判断期望对象是否存在 (newObj很可能存在但是期望值不存在就认为不存在 buildingNumber属于楼栋号)
                        ...
                        if (examineBuilding_.prototype.isEmpty(item)) {
                            ...
                            newObj.floorPlanAttachmentId = null;//把上部分复制的附件id设为null
                            newObj.figureOutsideAttachmentId = null;//把上部分复制的附件id设为null
                            newObj.floorAppearanceFigureAttachmentId = null;//把上部分复制的附件id设为null
                        }
                        //处理复制的子类 (直接在服务端处理)
                        $.ajax({
                        ...
                        //处理复制的附件信息
                        var fileFloorPlanAttachmentId = data.floorPlanAttachmentId;
                        var fileFigureOutsideAttachmentId = data.figureOutsideAttachmentId;
                        var fileFloorAppearanceFigureAttachmentId = data.floorAppearanceFigureAttachmentId;
                        $.ajax({
                ...
                tempSaveData: function (number) {
                    ...
                    var fileFloorPlanAttachmentId = examineBuilding_.prototype.getFloorPlanAttachmentId();
                    var fileFigureOutsideAttachmentId = examineBuilding_.prototype.getFigureOutsideAttachmentId();
                    var fileFloorAppearanceFigureAttachmentId = examineBuilding_.prototype.getFloorAppearanceFigureAttachmentId();
                    if (examineBuilding_.prototype.isEmpty(fileFloorPlanAttachmentId)) {
                        data.floorPlanAttachmentId = fileFloorPlanAttachmentId;
                        AssessCommon.getSysAttachmentDto(fileFloorPlanAttachmentId, function (fileData) {
                    ...
                    if (examineBuilding_.prototype.isEmpty(fileFigureOutsideAttachmentId)) {
                        ...
                        AssessCommon.getSysAttachmentDto(fileFigureOutsideAttachmentId, function (fileData) {
                            //这里需要注意:假如数据里面已经存储了附件id 那么不再更新
                            fileData.fieldsName = examineBuilding_.prototype.config().building_figure_outside + data.identifier;
                            AssessCommon.saveAndUpdateSysAttachmentDto(fileData, function (item) {
                                if (item != null) {
                                    examineBuilding_.prototype.setFigureOutsideAttachmentId(null);
                                }
                    ...
                    if (examineBuilding_.prototype.isEmpty(fileFloorAppearanceFigureAttachmentId)) {
                        ...
                        AssessCommon.getSysAttachmentDto(fileFloorAppearanceFigureAttachmentId, function (fileData) {
                            //这里需要注意:假如数据里面已经存储了附件id 那么不再更新
                            fileData.fieldsName = examineBuilding_.prototype.config().building_floor_Appearance_figure + data.identifier;
                            AssessCommon.saveAndUpdateSysAttachmentDto(fileData, function (item) {
                                if (item != null) {
                                    examineBuilding_.prototype.setFloorAppearanceFigureAttachmentId(null);
                                }
                ...
                initData: function (identifierNumber) {
                    ...
                    if (examineBuilding_.prototype.isEmpty(data)) {
                        ...
                        //显示附件
                        examineBuilding_.prototype.file.newFileShows(examineBuilding_.prototype.config().building_floor_plan, examineBuilding_.prototype.config().building_floor_plan + data.identifier);
                        examineBuilding_.prototype.file.newFileShows(examineBuilding_.prototype.config().building_figure_outside, examineBuilding_.prototype.config().building_figure_outside + data.identifier);
                        examineBuilding_.prototype.file.newFileShows(examineBuilding_.prototype.config().building_floor_Appearance_figure, examineBuilding_.prototype.config().building_floor_Appearance_figure + data.identifier);
                    }
                    examineBuilding_.prototype.sonModelMethod.buildingOutfit.loadDataDicList();
                    examineBuilding_.prototype.sonModelMethod.buildingFunction.loadDataDicList();
                },
            ...
            file: {
                ...
                init: function () {
                    ...
                    examineBuilding_.prototype.file.uploadFilesModel(examineBuilding_.prototype.config().building_figure_outside);
                    examineBuilding_.prototype.file.uploadFilesModel(examineBuilding_.prototype.config().building_floor_Appearance_figure);
                },
                //默认上传附件方法
