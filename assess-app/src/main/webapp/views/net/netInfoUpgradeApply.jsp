            ...
            <div class="page-inner mt--5">
                <div class="row mt--2">
                            ...
                            <div class="card-body">
                                <form id="master_form" class="form-horizontal">
                                    ...
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <input type="hidden" name="id" value="${netInfoUpgrade.id}">
                                            <c:if test="${netInfoUpgrade.type eq '房产'}">
                                                <table class="table table-bordered" id="houseHistory">
                                                    <!-- cerare document add ajax data-->
                                                </table>
                                            </c:if>
                                            <c:if test="${netInfoUpgrade.type eq '土地'}">
                    ...
                    <div class="col-md-12" style="text-align: center;padding-bottom: 1.25rem">
                        ...
                        <div class="card-body">
                            <button id="cancel_btn" type="button" class="btn btn-default" onclick="window.close()">
                                取消
                            </button>

                            <button id="commit_btn" type="button" class="btn btn-primary" style="margin-left: 10px;" onclick="masterObj.commit();">
                                提交
                            </button>
                        </div>
                    </div>
        ...
        <div class="modal-content">
                        ...
                        <div class="col-md-12">
                            <div class="card-body">
                                ...
                                <div class="landContent" style="display: none">
                                    <input type="hidden" name="type" value="土地">
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                ...
                                <div class="houseContent" style="display: none">
                                    <input name="type" type="hidden" value="房产">
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <div class="form-inline x-valid">
                                                <label class="col-sm-1 col-form-label">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary btn-sm"
                        onclick="detailInfo.prototype.saveJsonData()">
                    保存
                </button>
            </div>
...
<script type="text/javascript">
    $(function () {
        ...
        if('${netInfoUpgrade.jsonData}'){
            console.log("1234")
            $("#jsonData").val('${netInfoUpgrade.jsonData}')
        }
    })
    ...
    function commit() {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/netInfoUpgrade/applyCommit",
            type: "post",
            data: {id: id},
    ...
    function editCommit() {
        ...
        Loading.progressShow();
        $.ajax({
            url: "${pageContext.request.contextPath}/netInfoUpgrade/editCommit",
            type: "post",
    ...
    detailInfo.prototype = {
        ...
        saveJsonData: function () {
            ...
            data.dealPartInfo = $("#" + detailInfo.prototype.config().frm).find("#landDealPartInfo").val();
            $.ajax({
                url: "${pageContext.request.contextPath}/netInfoUpgrade/saveJsonData",
                type: "post",
                dataType: "json",
                data: {
                    jsonData: jsonData,
                    id: ${netInfoUpgrade.id}
                },
                success: function (result) {
                    if (result.ret) {
        ...
        loadLandHistoryList: function () {
            ...
            cols.push({
                field: 'id', width: '6%', title: '操作', formatter: function (value, row, index) {
                    var str = '<div class="btn-margin">';
                    str += '<button type="button" style="margin-left: 5px;"  class="btn btn-xs btn-primary tooltips"  data-placement="top" data-original-title="升级" onclick="detailInfo.prototype.getAndInitLand(' + row.id + ')"><i class="fa fa-pen fa-white"></i></button>';
                    str += '</div>';
                    return str;
                }
            });
            $("#landHistory").bootstrapTable('destroy');
            TableInit("landHistory", "${pageContext.request.contextPath}/netInfoUpgrade/landList", cols, {
                id: ${netInfoUpgrade.dataId},
                applyId:'${netInfoUpgrade.id}'
            }, {
                showColumns: false,
                showRefresh: false,
                search: false,
        ...
        getAndInitLand: function (id) {
            detailInfo.prototype.loadOnclickData(${netInfoUpgrade.masterDataId});
            $.ajax({
                ...
                success: function (result) {
                    if (result.ret) {
                        ...
                        detailInfo.prototype.showContent(result.data.type, result)
        ...
        loadHouseHistoryList: function () {
            ...
            cols.push({
                field: 'id', width: '6%', title: '操作', formatter: function (value, row, index) {
                    var str = '<div class="btn-margin">';
                    str += '<button type="button" style="margin-left: 5px;"  class="btn btn-xs btn-primary tooltips"  data-placement="top" data-original-title="升级" onclick="detailInfo.prototype.getAndInitHouse(' + row.id + ')"><i class="fa fa-pen fa-white"></i></button>';
                    str += '</div>';
                    return str;
                }
            });
            $("#houseHistory").bootstrapTable('destroy');
            TableInit("houseHistory", "${pageContext.request.contextPath}/netInfoUpgrade/houseList", cols, {
                id: ${netInfoUpgrade.dataId},
                applyId:'${netInfoUpgrade.id}'
            }, {
                showColumns: false,
        ...
        showContent: function (type, result) {
