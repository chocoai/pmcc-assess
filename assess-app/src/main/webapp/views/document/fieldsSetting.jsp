    ...
    function loadTemplateFieldList() {
        ...
        cols.push({
            field: 'opation',width:"10%", title: '操作', formatter: function (value, row, index) {
                var str = '<button onclick="editTemplateFields(' + index + ')"  style="margin-left: 5px;"  class="btn btn-icon btn-primary  btn-xs tooltips"  data-placement="bottom" data-original-title="编辑">';
                str += '<i class="fa fa-pen"></i>';
                str += '</button>';
                str += '<button onclick="deleteTemplateFields(' + row.id + ',\'tb_List\')"  style="margin-left: 5px;"  class="btn btn-icon btn-warning  btn-xs tooltips"  data-placement="bottom" data-original-title="删除">';
