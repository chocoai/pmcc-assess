...
(function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showRefresh) {
            html.push(sprintf('<button class="btn' +
                    sprintf(' btn-%s', this.options.buttonsClass) +
                    sprintf(' btn-%s', this.options.iconSize) +
                    '" type="button" name="refresh" aria-label="search" title="%s">',
        ...
        if (this.options.showRefresh) {
            this.$toolbar.find('button[name="refresh"]')
                ...
                function () {
                    var obj = $(".bootstrap-table .search input");
                    var text = $(obj).val();
                    that.searchText = text;
                    that.options.searchText = text;
                    that.options.pageNumber = 1;
                    that.initSearch();
                    that.updatePagination();
                    that.trigger('search', text);
                }
            );
        }
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if (this.options.showTotalPages) {
            html.push(
        ...
        if (!this.options.onlyInfoPagination) {
            ...
            html.push('<div class="dataTables_paginate paging_simple_numbers col-sm-7">',
                '<ul class="pagination' + sprintf(' pagination-%s', this.options.iconSize) + '" style="margin: 0px;">',
